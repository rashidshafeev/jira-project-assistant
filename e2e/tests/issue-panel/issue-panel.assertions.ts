...
/**
 * After assigning from the panel: the inline Fix form collapses (unmounts), the issue
 * re-reads, its only problem is gone (panel flips to "healthy", Fix button disappears),
 * and the new assignee is shown — so the panel reflects the write, not stale cached state.
 */
export async function expectResolvedPanel(app: AppPage, d: IssuePanelData): Promise<void> {
  await expect(app.panelFix()).toHaveCount(0)
